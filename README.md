...
Run the cells in order in one kernel session — Restart Kernel + Run All is the safe move. The trained `.keras` file isn't committed (it's big and regenerates from the notebook), so it's gitignored.
